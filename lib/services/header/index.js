...
HeaderService.prototype.getCurrentDifficulty = function() {
  var target = vcoin.mining.common.getTarget(this._lastHeader.bits);
  return vcoin.mining.common.getDifficulty(target);
};
...
HeaderService.prototype._queueBlock = function(block) {
  ...
  self._blockProcessor.push(block, function(err) {
    ...
    log.debug('Header Service: completed processing block: ' + block.rhash() + ' prev hash: ' + vcoin.util.revHex(block.prevBlock));
...
HeaderService.prototype._detectReorg = function(block) {
  return vcoin.util.revHex(block.prevBlock) !== this._lastHeader.hash;
};
