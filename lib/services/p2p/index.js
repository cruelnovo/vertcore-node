...
var Vcoin = require('./bcoin');
var VcoinTx = require('vcoin').tx;
...
P2P.prototype.sendTransaction = function(tx, callback) {
  ...
  var vcoinTx;
  try {
    vcoinTx = VcoinTx.fromRaw(tx, 'hex');
  } catch(e) {
    return callback(e);
  }

  log.info('P2P Service: sending transaction: ' + vcoinTx.txid());

  this._outgoingTxs.set(vcoinTx.txid(), vcoinTx);
  var inv = p2p.Inventory.forTransaction(vcoinTx.txid());
  var txMessage = this.messages.Inventory([inv]);

  peer.sendMessage(txMessage);

  this._onPeerTx(peer, { transaction: vcoinTx });

  return callback(null, vcoinTx.txid());
};


P2P.prototype.start = function(callback) {
  var self = this;
  self._startVcoinIfNecessary(function(){
    self._initCache();
    self._initPool();
    self._setListeners();
    callback();
...
P2P.prototype.stop = function(callback) {
  ...
  if (this._vcoin){
    return this._vcoin.stop(callback);
  }
...
P2P.prototype._initP2P = function() {
  ...
  this.messages = new p2p.Messages({ network: Networks.get(this.node.network), Transaction: VcoinTx });
  this._peerHeights = [];
  this._peers = [];
  this._peerIndex = 0;
  this._mempoolFilter = [];
...
P2P.prototype._onPeerGetData = function(peer, message) {
  ...
  if (tx) {
    peer.sendMessage(this.messages.Transaction(tx, { Transaction: VcoinTx }));
  }
};
...
P2P.prototype._onPeerInventory = function(peer, message) {
  ...
  message.inventory.forEach(function(inv) {
    ...
    if (!self._inv.get(inv.hash)) {
      ...
      if (inv.type === p2p.Inventory.TYPE.BLOCK || inv.type === p2p.Inventory.TYPE.TX || inv.type === p2p.Inventory.TYPE.FILTERED_BLOCK) {
        inv.type |= p2p.Inventory.FLAG.WITNESS;
      }
...
P2P.prototype._startVcoin = function(callback) {
  ...
  if (['livenet', 'live', 'main', 'mainnet'].indexOf(this.node.network) !== -1) {
    network = 'main';
    port = this._configPeers[0].port || 5889;
  } else if (this.node.network !== 'regtest') {
    network = 'testnet';
    port = this._configPeers[0].port || 15889;
  } else {
    network = this.node.network;
    port = this._configPeers[0].port || 18444;
  }

  self._vcoin = new Vcoin({
    network: network,
    prefix: self.node.datadir,
    port: port
  });

  self._vcoin.start(callback);

};

P2P.prototype._startVcoinIfNecessary = function(callback) {
  if (!this._hasPeers()) {
    log.info('Peers not explicitly configured, starting a local vcoin node.');
    this._configPeers = [{ ip: { v4: '127.0.0.1'} }];
    return this._startVcoin(callback);
  }
  setImmediate(callback);
};
