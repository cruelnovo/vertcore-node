...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both vertcoin.conf and vertcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/Cubey2019/vertcore-node/blob/bitcoind/docs/upgrade.md \n');
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './vertcore-node.json');
...
function loadModule(req, service) {
  try {
    ...
    if (servicePackage.vertcoreNode) {
        serviceModule = service.name + '/' + servicePackage.vertcoreNode;
    } else if (servicePackage.bitcoreNode) {
 ...
 * @param {Node} node - The Vertcore Node instance
 */
