...
function lookInModuleManifest(req, service) {
  try {
    ...
    if (servicePackage.vertcoreNode) {
      serviceModule = serviceModule + '/' + servicePackage.vertcoreNode;
...
function loadModule(req, service) {
  ...
  //package.json file, if so, then see if there is a vertcoreNode field, if so
  ...
  if (!serviceCode) {
    throw new Error('Attempted to load the ' + service.name + ' service from: ' +
      ...
      process.cwd() + '/package.json" - vertcoreNode field. All paths failed to find valid nodeJS code.');
  }
