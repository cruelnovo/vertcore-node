...
/**
 * Will return the path and default vertcore-node configuration. It will search for the
 * configuration file in the "~/.vertcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.vertcore');
  var defaultConfigFile = path.resolve(defaultPath, './vertcore-node.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    console.log(`The configuration file at '${defaultConfigFile}' is incompatible with this version of Vertcore.`);

    var now = new Date();
    // vertcore-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `vertcore-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
  ...
  var defaultConfig = {
    ...
    servicesConfig: {
      'insight-vtc-api': {
        cwdRequirePath: 'node_modules/insight-vtc-api'
      },
      'insight-vtc-ui': {
        cwdRequirePath: 'node_modules/insight-vtc-ui'
      }
    }
  };
